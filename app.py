...
# --- 3. FUNGSI EVALUASI (DIPERBAIKI) ---
def evaluasi_row(uraian, kode_user):
    # Cek jika uraian kosong
    if pd.isna(uraian): return None
    
    # --- PERBAIKAN DI SINI ---
    # Gunakan variabel 'uraian', bukan 'text'
    text_diperkaya = f"Belanja Pengadaan {uraian}" 
    
    # Encode & Search menggunakan teks yang sudah diperkaya
    query_vec = model.encode(str(text_diperkaya), convert_to_tensor=True)
    
    # Lanjutan logika sama seperti sebelumnya...
